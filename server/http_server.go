...
func Upload(env *Env, tag string, body []byte) bool {
	...
	t := time.Now()
	objectKey := GenerateObjectKey(env.S3Prefix, env.S3TrailingPrefix, tag, t)
...
func GenerateObjectKey(S3Prefix, S3TrailingPrefix string, tag string, t time.Time) string {
	timestamp := t.Format("20060102150405")
	date := t.Format("20060102")
	hour := strconv.Itoa(t.Hour())
	fileName := strings.Join([]string{timestamp, "_", tag, ".log"}, "")
